...
function renderProducts(data) {
  data.products.forEach(function (item) {
    markup += "\n    <div class=\"product\">\n      <div class=\"product__div-img\">\n        <img src=\"http:".concat(item.image, "\" alt=\"").concat(item.name, "\" class=\"product__img\">\n      </div>\n      <h5 class=\"product__name\">").concat(item.name, "</h5>\n      <p class=\"product__description\">\n        ").concat(item.description, "\n      </p>\n      <p class=\"product__old-price\">De: R$").concat(item.oldPrice, ",00</p>\n      <h4 class=\"product__new-price\">Por: R$").concat(item.price, ",00</h4>\n      <p class=\"product__payment\">ou ").concat(item.installments.count, "x de R$").concat(item.installments.value.toFixed(2).toString().replace('.', ','), "</p>\n      <button id=\"").concat(item.id, "\" class=\"product__btn-purchase\">Comprar</button>\n    </div>\n    ");
  });
...
function renderButtonMore(data) {
  ...
  var btnListMore = document.querySelector('#btn__list-more');
  btnListMore.addEventListener('click', function () {
    fetch("https:".concat(data.nextPage)).then(function (data) {
      return data.json();
    }).then(function (data) {
      console.log(data);
      renderProducts(data);
...
var controlName = document.querySelector('.controlName');
var controlEmail = document.querySelector('.controlEmail');
